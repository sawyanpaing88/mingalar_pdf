...
# ==========================================
# 🏠 DASHBOARD ENGINE (WITH EXPANDED VIEW TABLE)
# ==========================================
if page_selection == "🏠 Dashboard Console":
    ...
    if not df_quotes.empty:
        ...
        for idx, row in df_quotes.iterrows():
            ...
            # Expanded View Panel rendering details as an interactive table
            if st.session_state.get(f"expanded_view_{row['id']}", False):
                with st.container():
                    ...
                    try:
                        ...
                        # Filter to standard user visible metadata columns
                        display_cols = ["No", "Part Number/Model", "Description", "Qty", "Unit Price", "Margin", "Total Price"]
                        for col in display_cols:
                            if col not in df_items.columns:
                                df_items[col] = ""
                        
                        # Render static table list context mapping layout
...
# ==========================================
    ...
    # Global adjustments configuration hook
    ...
    # Math recalculation execution block
    for item in st.session_state.working_items:
    ...
    if not edited_df.equals(df_display[visible_cols]):
        ...
        for idx, row in edited_df.iterrows():
            ...
            updated_records.append({
                ...
                "Margin": float(target_margin), "Total Price": 0.0
            })
    ...
    if action_c2.button("🖨️ Compile Official Corporate PDF Engine Asset"):
        ...
        <head>
            ...
            <style>
                ...
                .logo-img {{ max-height: 55px; display: inline-block; }}
                .address-text {{ font-size: 8pt; color: #4b5563; text-align: right; line-height: 1.4; width: 50%; }}
                ...
                .items-table th {{ background-color: #1f2937; color: white; padding: 10px 8px; font-size: 9pt; text-transform: uppercase; border: 1px solid #1f2937; font-weight: bold; }}
                .items-table td {{ border: 1px solid #e5e7eb; padding: 8px; vertical-align: top; font-size: 8.5pt; word-wrap: break-word; overflow: visible; }}
                .col-no {{ width: 7%; }} .col-part {{ width: 22%; }} .col-desc {{ width: 33%; }} .col-qty {{ width: 8%; }} .col-uprice {{ width: 15%; }} .col-tprice {{ width: 15%; }}
        ...
        <body>
            <table class="brand-header">
                <tr>
                    <td class="logo-container"><img src="https://arktechsolutions.net/wp-content/themes/wp-ark/assets/img/logo-ark.png" class="logo-img"></td>
            ...
            <table class="meta-table">
                <tr>
                    <td><strong>Prepared For:</strong><br><span style="font-size: 10pt; font-weight: bold; color: #111;">{client_company}</span><br>Attn: {attn_person}<br>Email: {attn_email} | Tel: {attn_phone}</td>
                    <td><strong>Reference Context Ledger:</strong><br>Quotation ID Ref: <strong>{quotation_auto_gen}</strong><br>Generation Timestamp: {issue_date}<br>Validity Scope Limit: {validity_bound}<br>Base Operational Currency: {currency_selection}</td>
                </tr>
            </table>
            {exchange_rate_notice}
            <table class="items-table">
                <thead>
                    <tr><th class="col-no">No</th><th class="col-part">Part Number</th><th class="col-desc">Description</th><th class="col-qty">Qty</th><th class="col-uprice">Unit Price</th><th class="col-tprice">Total Price</th></tr>
                </thead>
                <tbody>{html_table_rows}</tbody>
            </table>
            <div class="totals-container">
                <table class="totals-table">
                    <tr><td>Gross Framework Subtotal:</td><td style="text-align: right; font-weight: 500;">{currency_symbol}{global_subtotal_base:,.2f}</td></tr>
                    <tr><td>Global Aggregate Discount Adjustment:</td><td style="text-align: right; color: #dc2626;">-{currency_symbol}{global_discount_base:,.2f}</td></tr>
                    <tr><td>Commercial Tax Pool Value ({global_tax_pct}%):</td><td style="text-align: right;">{currency_symbol}{calculated_tax:,.2f}</td></tr>
                    <tr class="grand-total-row"><td>Grand Total ({currency_selection}):</td><td style="text-align: right; font-size: 10.5pt;">{currency_symbol}{calculated_grand_total:,.2f}</td></tr>
                </table>
            </div>
            <div class="footer-notes">
                <strong>Commercial Delivery Terms & Scope:</strong><br>Estimated Production Delivery Lead Time: {lead_time_frame}<br>Target Execution Payment Schedule Terms: {payment_terms_desc}<br><br>
                <strong>Operational Provisions & Custom Legal Scope:</strong><br>{terms_and_cond.replace('\n', '<br>')}
            </div>
        </body>
        </html>
        """
