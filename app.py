...
from pypdf import PdfReader  # Added for PDF processing matrix
...
# New Parser implementation targeting native text strings inside PDFs
    ...
    try:
        ...
        for page in reader.pages:
            ...
            for line in text.split('\n'):
                line_str = line.strip()
                # Skip header lines, totals, or empty structures
                if not line_str or "total" in line_str.lower() or "description" in line_str.lower():
                    continue
                
                # Check for currency structures or number metrics to guess price
                ...
                # Simple partition logic: Grab the front word cluster as Part Number if alphanumeric
...
if page_selection == "🏠 Dashboard Console":
    ...
    if not df_quotes.empty:
        ...
        if current_user["role"] == "Top Management":
            ...
            st.dataframe(filtered_df[["po_number", "customer_name", "project_name", "status", "grand_total", "creator_email"]], width="stretch")
        else:
            st.dataframe(df_quotes[["po_number", "customer_name", "project_name", "status", "grand_total"]], width="stretch")
    else:
...
if page_selection == "➕ Build New Quotation Module":
    ...
    # UPDATED: Added target array criteria constraints mapping for PDF, XLSX, XLS, and CSV input channels
    ...
    if action_c2.button("🖨️ Compile Official Corporate PDF Engine Asset"):
        ...
        for idx, r in edited_items_df.iterrows():
            is_sub = "." in str(r["No"])
            indent_style = "padding-left: 15px; font-style: italic; color: #475569;" if is_sub else "font-weight: bold;"
            ...
            <tr>
                ...
                <td style="width: 22%; font-size: 8pt;">{r['Part Number/Model']}</td>
                <td style="width: 40%; font-size: 8pt; {indent_style}">{r['Description']}</td>
